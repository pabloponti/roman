	...
	function columns_data( $column ) {
		...
		switch( $column ) {
			...
			case "listing_tags":
				...
				echo (!empty($status))?'<b>Estado</b>: ' .$status . '<br /><br />':'';
				$propertytype = get_the_term_list( $post->ID, 'property-types', '', ', ', '' );
				echo (!empty($propertytype))?'<b>Tipo de Propiedad:</b> ' .$propertytype . '<br /><br />':'';
				$locations = get_the_term_list( $post->ID, 'locations', '', ', ', '' );
				echo (!empty($locations))?'<b>Ubicación:</b> ' .$locations . '<br /><br />':'';
				$features = get_the_term_list( $post->ID, 'features', '', ', ', '' );
				echo (!empty($features))?'<b>Caracteristicas:</b><br />' .$features . '<br /><br />':'';
				break;
		}
