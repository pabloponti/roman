...
function single_listing_post_content() {
	...
	<div itemscope itemtype="http://schema.org/SingleFamilyResidence" class="entry-content wplistings-single-listing">
		...
		if ( '' != wp_listings_get_property_types() ) {
			$listing_meta .= sprintf( '<li class="listing-property-type"><span class="label">Tipo de Propiedad: </span>%s</li>', get_the_term_list( get_the_ID(), 'property-types', '', ', ', '' ) );
		}

		if ( '' != wp_listings_get_locations() ) {
			$listing_meta .= sprintf( '<li class="listing-location"><span class="label">Ubicación: </span>%s</li>', get_the_term_list( get_the_ID(), 'locations', '', ', ', '' ) );
		}

		if ( '' != get_post_meta( $post->ID, '_listing_bedrooms', true ) ) {
			$listing_meta .= sprintf( '<li class="listing-bedrooms"><span class="label">Habitaciones: </span>%s</li>', get_post_meta( $post->ID, '_listing_bedrooms', true ) );
		}

		if ( '' != get_post_meta( $post->ID, '_listing_bathrooms', true ) ) {
			$listing_meta .= sprintf( '<li class="listing-bathrooms"><span class="label">Baños: </span>%s</li>', get_post_meta( $post->ID, '_listing_bathrooms', true ) );
		}

		if ( '' != get_post_meta( $post->ID, '_listing_sqft', true ) ) {
			$listing_meta .= sprintf( '<li class="listing-sqft"><span class="label">Metros Cuadrados: </span>%s m²</li>', get_post_meta( $post->ID, '_listing_sqft', true ) );
		}

		if ( '' != get_post_meta( $post->ID, '_listing_lot_sqft', true ) ) {
			$listing_meta .= sprintf( '<li class="listing-lot-sqft"><span class="label">Metros Cuadrados (lote): </span>%s m²</li>', get_post_meta( $post->ID, '_listing_lot_sqft', true ) );
		}
		...
		<div id="listing-tabs" class="listing-data">
			...
			<ul>
				...
				<?php if (get_post_meta( $post->ID, '_listing_gallery', true) != '') { ?>
					<li><a href="#listing-gallery">Fotos</a></li>
				<?php } ?>

				<?php if (get_post_meta( $post->ID, '_listing_video', true) != '') { ?>
					<li><a href="#listing-video">Video / Tour Virtual</a></li>
				<?php } ?>
			...
			<div id="listing-details">
				<?php
                    ...
                    echo (!empty($price))?'<tr class="wp_listings_listing_price"><td class="label">Precio:</td><td>'.$price .'</td></tr>':'';
                    echo '<div itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">';
                    $address = get_post_meta( $post->ID, '_listing_address', true);
                    echo (!empty($address))?'<tr class="wp_listings_listing_address"><td class="label">Dirección:</td><td itemprop="streetAddress">'.$address .'</td></tr>':'';                    
                    $city = get_post_meta( $post->ID, '_listing_city', true);
                    echo (!empty($city))?'<tr class="wp_listings_listing_city"><td class="label">Ciudad:</td><td itemprop="addressLocality">'.$city .'</td></tr>':'';
                    $state = get_post_meta( $post->ID, '_listing_state', true);
                    echo (!empty($state))?'<tr class="wp_listings_listing_state"><td class="label">Región:</td><td itemprop="addressRegion">'.$state .'</td></tr>':'';
                    $zip = get_post_meta( $post->ID, '_listing_zip', true);
                    echo (!empty($zip))?'<tr class="wp_listings_listing_zip"><td class="label">Código Postal:</td><td itemprop="postalCode">'.$zip .'</td></tr>':'';
                    echo '</div>';
                    $mls = get_post_meta( $post->ID, '_listing_mls', true);
                    echo (!empty($mls))?'<tr class="wp_listings_listing_mls"><td class="label">MLS:</td><td>'.$mls .'</td></tr>':'';
                    echo '</tbody>';
		...
		<div id="listing-contact" <?php if(!function_exists('aeprofiles_connected_agents_markup')) { echo 'style="width: 100%;"'; }; ?>>
			...
			<?php if(isset($emailSent) && $emailSent == true) {	?>
				<div class="thanks">
					<a name="redirectTo"></a>
					<p>Gracias, su correo electrónico fue enviado! Estaremos en contacto en breve.</p>
				</div>
			<?php } else { ?>
				<?php if(isset($hasError)) { ?>
					<a name="redirectTo"></a>
					<label class="error" name="redirectTo">Lo sentimos, ha ocurrido un error. Por favor, vuelva a intentarlo.<label>
				<?php } ?>
				...
				<form action="<?php the_permalink(); ?>#redirectTo" id="inquiry-form" method="post">
					<ul class="inquiry-form">
						...
						<li class="contactComments"><label for="commentsText">Consulta:</label>
